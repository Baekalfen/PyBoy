...
def addPopToStack(s):
    try:
        ...
        pass
        # gblogger.info("Tried to pop from empty call stack")
...
opcode_Dx = [(1, (20, 8), lambda s, e: (addPopToStack(s) or s.setPC(s.CPU_RET())) if not s.testFlag(flagC) else s.setPC(e)),                   # D0 -  "Return if last result caused no carry" ("RET NC")
             ...
             (1, (16,), lambda s, _: s.CPU_EI() or addPopToStack(s) or s.setPC(s.CPU_RET())),                          # D9 -  "Enable interrupts and return to calling routine" ("RETI")
