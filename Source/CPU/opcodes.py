...
# Noter: Vi har valgt look-up-table, da antallet af registre er forholdsvis begrænset.
#       Hvis der havde være flere registre kunne man med fordel bruge bitmasking i stedet.

# http://gamehacking.org/faqs/GameBoy_Z80_Opcode_Map.html
# http://www.pastraiser.com/cpu/gameboy/gameboy_opcodes.html

# Maybe http://stackoverflow.com/a/36033/1796585 ??

# from PrimitiveTypes import uint8, uint16
...
# class Token:
#    def __repr__(self):
#        return "CPU_" + hex(self.value) + ")"


# class Register(Token):
#    def __repr__(self):
#        return "Register(" + hex(self.value) + ")"


def pointer(self, p):
    return self.ram[p.getInt()]
...
#    def __repr__(self):
#        return "Parameter(" + hex(self.value) + ")"


# from operations import CPU_NOP, CPU_EI, CPU_STOP, CPU_HALT, CPU_LDD, CPU_LDI, CPU_LDH, CPU_LDHL, CPU_INC8, CPU_DEC8, CPU_INC16, CPU_DEC16, CPU_ADD, CPU_SUB, CPU_ADC, CPU_SBC, CPU_AND, CPU_XOR, CPU_OR, CPU_CP, CPU_RLC, CPU_RRC, CPU_RL, CPU_RR, CPU_DAA, CPU_CPL, CPU_SCF, CPU_CCF, CPU_JR, CPU_POP, CPU_PUSH, CPU_JP, CPU_CALL, CPU_RET, CPU_RETI, CPU_RST, CPU_DI, CPU_Ext, CPU_EXT_SLA, CPU_EXT_SRA, CPU_EXT_SWAP, CPU_EXT_SRL, CPU_EXT_BIT, CPU_EXT_RES, CPU_EXT_SET, CPU_EXT_RLC, CPU_EXT_RRC, CPU_EXT_RL, CPU_EXT_RR
# from registers import reg, A, F, B, C, D, E, H, L, SP, PC, setAF, setBC, setDE, setHL
...
opcodes = []
#           Tokens                                       opcode   Description

#TODO: Fix all JR to use CONDITION and (setPC(...) or True)
...
opcode_0x = [(1, (4,), lambda s, e: s.setPC(e)),                      # 00 - "No Operation" ("NOP")
             ...
             (1, (8,), lambda s, e: s.ram.__setitem__(s.getBC(), s.reg[A]) or s.setPC(e)),  # 02 - "Save A to address pointed by BC" ("LD(s.BC),A")
             ...
             (3, (20,), lambda s, v, e: s.ram.__setitem__(v, s.reg[SP] & 0xFF) or s.ram.__setitem__(v+1, (s.reg[SP] & 0xFF00) >> 8) or s.setPC(e)),  # 08 - "Save SP to given address" ("LD (v), s.SP")
             (1, (8,), lambda s, e: s.setHL(s.CPU_ADD16(s.getHL(), s.getBC())) or s.setPC(e)),    # 09 - "Add 16-bit BC to HL" ("ADD HL,BC")
             (1, (8,), lambda s, e: s.setReg(A, s.ram[s.getBC()]) or s.setPC(e)),   # 0A - "Load A from address pointed to by BC" ("LD A,(s.BC)")
...
opcode_1x = [(1, (4,), lambda s, e: s.CPU_STOP(e)),                           # 10 - "Stop processor" ("STOP")
             ...
             (1, (8,), lambda s, e: s.ram.__setitem__(s.getDE(), s.reg[A]) or s.setPC(e)),   # 12 - "Save A to address pointed by DE" ("LD(s.DE),A")
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.ram[s.getDE()]) or s.setPC(e)),   # 1A - "Load A from address pointed to by DE" ("LD A, s.DE")
...
opcode_2x = [(2, (12, 8), lambda s, v, e: s.setPC(e+getSignedInt8(v)) if not s.testFlag(flagZ) else s.setPC(e)),      # 20 - "Relative jump by signed immediate if last result was not zero" ("JR NZ,r8")
             ...
             (1, (8,), lambda s, e: s.ram.__setitem__(s.getHL(), s.reg[A]) or (s.setHL(s.getHL() + 1) or s.setPC(e))),  # 22 - "Save A to address pointed by HL, and increment HL" ("LDI (HL),A")
...
opcode_3x = [(2, (12, 8), lambda s, v, e: (not s.testFlag(flagC) and (s.setPC(e+getSignedInt8(v)) or True)) or s.setPC(e)),      # 30 - "Relative jump by signed immediate if last result caused no carry" ("JR NC,r8")
             ...
             (1, (8,), lambda s, e:(s.ram.__setitem__(s.getHL(), s.reg[A]) or s.setHL(s.getHL()-1)) or s.setPC(e)),  # 32 - "Save A to address pointed by HL, and decrement HL" ("LDD (HL),A")
             (1, (8,), inc16(SP)),                 # 33 - "Increment 16-bit HL" ("INC SP")
             (1, (12,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_INC8(s.ram[s.getHL()])) or s.setPC(e)),  # 34 - "Increment value pointed by HL" ("INC (HL)
             (1, (12,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_DEC8(s.ram[s.getHL()])) or s.setPC(e)),  # 35 - "Decrement value pointed by HL" ("DEC (HL)
             (2, (12,), lambda s, v, e: s.ram.__setitem__(s.getHL(), v) or s.setPC(e)),  # 36 - "Load 8-bit immediate into address pointed by HL" ("LD (HL),v")
             ...
             (1, (8,), lambda s, e: (s.setReg(A, s.ram[s.getHL()]) or s.setHL(s.getHL()-1)) or s.setPC(e)),  # 3A - "Load A from address pointed to by HL, and decrement HL" ("LDD A,(HL)
...
opcode_4x = [(1, (4,), exeLD(B, B)),        # 40 - "Copy B to B" ("LD B,B")
             ...
             (1, (8,), lambda s, e: s.setReg(B, s.ram[s.getHL()]) or s.setPC(e)),  # 46 - "Copy value pointed by HL to B" ("LD B,(HL)
             ...
             (1, (8,), lambda s, e: s.setReg(C, s.ram[s.getHL()]) or s.setPC(e)),  # 4E - "Copy value pointed by HL to C" ("LD C,(HL)
...
opcode_5x = [(1, (4,), exeLD(D, B)),        # 50 - "Copy B to D" ("LD D,B")
             ...
             (1, (8,), lambda s, e: s.setReg(D, s.ram[s.getHL()]) or s.setPC(e)),  # 56 - "Copy value pointed by HL to D" ("LD D,(HL)
             ...
             (1, (8,), lambda s, e: s.setReg(E, s.ram[s.getHL()]) or s.setPC(e)),  # 5E - "Copy value pointed by HL to E" ("LD E,(HL)
...
opcode_6x = [(1, (4,), exeLD(H, B)),        # 60 - "Copy B to H" ("LD H,B")
             ...
             (1, (8,), lambda s, e: s.setReg(H, s.ram[s.getHL()]) or s.setPC(e)),  # 66 - "Copy value pointed by HL to H" ("LD H,(s.H)
             ...
             (1, (8,), lambda s, e: s.setReg(L, s.ram[s.getHL()]) or s.setPC(e)),  # 6E - "Copy value pointed by HL to L" ("LD L,(s.H)
...
opcode_7x = [(1, (8,), exeLDaddr(B)),  # 70 - "Copy B to address pointed by HL" ("LD (HL),B")
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.ram[s.getHL()]) or s.setPC(e)),  # 7E - "Copy value pointed by HL to A" ("LD A,(HL)
...
opcode_8x = [(1, (4,), lambda s, e: s.setReg(A,s.CPU_ADD8(s.reg[A], s.reg[B])) or s.setPC(e)),                     # 80 - "Add B to A" ("ADD A,B")
             ...
             (1, (8,), lambda s, e: s.setReg(A,s.CPU_ADD8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),  # 86 - "Add value pointed by HL to A" ("ADD A,(HL)
             ...
             (1, (8,), lambda s, e: s.setReg(A,s.CPU_ADC8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),  # 8E - "Add value pointed by HL and carry flag to A" ("ADC A,(HL)
...
opcode_9x = [(1, (4,), exeALUSub(A, B)),                  # 90 -  "Subtract B from A" ("SUB A,B")
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.CPU_SUB8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),  # 96 -  "Subtract value pointed by HL from A" ("SUB A,(HL)
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.CPU_SBC8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),  # 9E -  "Subtract value pointed by HL and carry flag from A" ("SBC A,(HL)
...
opcode_Ax = [(1, (4,), lambda s, e: s.setReg(A, s.CPU_AND8(s.reg[A], s.reg[B])) or s.setPC(e)),                  # A0 -  "Logical AND B against A" ("AND B")
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.CPU_AND8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),                 # A6 -  "Logical AND value pointed by HL against A" ("AND (HL)
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.CPU_XOR8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),                 # AE -  "Logical XOR value pointed by HL against A" ("XOR (HL)
...
opcode_Bx = [(1, (4,), lambda s, e: s.setReg(A, s.CPU_OR8(s.reg[A], s.reg[B])) or s.setPC(e)),                   # B0 -  "Logical OR B against A" ("OR B")
             ...
             (1, (8,), lambda s, e: s.setReg(A, s.CPU_OR8(s.reg[A], s.ram[s.getHL()])) or s.setPC(e)),           # B6 -  "Logical OR value pointed by HL against A" ("OR (HL)
             ...
             (1, (8,), lambda s, e: s.CPU_CP(s.reg[A], s.ram[s.getHL()]) or s.setPC(e)),  # BE -  "Compare value pointed by HL against A" ("CP (HL)
...
def addPopToStack(s):
    try:
        ...
        warnings.warn("Tried to pop from empty call stack")
...
opcode_Ex = [(2, (12,), lambda s, v, e: s.ram.__setitem__(0xFF00+v, s.reg[A]) or s.setPC(e)),  # E0 -  "Save A at address pointed to by (FF00h + 8-bit immediate)" ("LDH (a8),A")
             (1, (12,), lambda s, e: s.setHL(s.CPU_POP()) or s.setPC(e)),                # E1 -  "Pop 16-bit value from stack into HL" ("POP HL")
             (1, (8,), lambda s, e: s.ram.__setitem__(0xFF00+s.reg[C], s.reg[A]) or s.setPC(e)),  # E2 -  "Save A at address pointed to by (FF00h + s.C)" ("LDH(s.C),A")
             ...
             (3, (16,), lambda s, v, e: s.ram.__setitem__(v, s.reg[A]) or s.setPC(e)),  # EA -  "Save A at given 16-bit address" ("LD (v),A")
...
opcode_Fx = [(2, (12,), lambda s, v, e: s.setReg(A, s.ram[v+0xFF00]) or s.setPC(e)),  # F0 -  "Load A from address pointed to by (FF00h + 8-bit immediate)" ("LDH A,(a8)
             ...
             (2, (12,), lambda s, v, e: s.setReg(A, s.ram[0xFF00+s.reg[C]]) or s.setPC(e)),  # F2 - no$gmb says LD a, (0xFF00+c)
             ...
             (3, (16,), lambda s, v, e: s.setReg(A, s.ram[v]) or s.setPC(e)),  # FA -  "Load A from given 16-bit address" ("LD A,(v)
...
CB_prefix_0x = [(1, (8,), rlc(B)),          # 00 - "Rotate B left with carry"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RLC(s.ram[s.getHL()])) or s.setPC(e)),  # 06 - "Rotate value pointed by HL left with carry"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RRC(s.ram[s.getHL()])) or s.setPC(e)),  # 0E - "Rotate value pointed by HL right with carry"
...
CB_prefix_1x = [(1, (8,), rl(B)),           # 10 - "Rotate B left"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RL(s.ram[s.getHL()])) or s.setPC(e)),  # 16 - "Rotate value pointed by HL left"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RR(s.ram[s.getHL()])) or s.setPC(e)),  # 1E - "Rotate value pointed by HL right"
...
CB_prefix_2x = [(1, (8,), ext_sla(B)),          # 20 - "Shift B left preserving sign"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SLA(s.ram[s.getHL()])) or s.setPC(e)),  # 26 - "Shift value pointed by HL left preserving sign"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SRA(s.ram[s.getHL()])) or s.setPC(e)),  # 2E - "Shift value pointed by HL right preserving sign"
...
CB_prefix_3x = [(1, (8,), ext_swap(B)),         # 30 - "Swap nibbles in B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SWAP(s.ram[s.getHL()])) or s.setPC(e)),  # 36 - "Swap nibbles in value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SRL(s.ram[s.getHL()])) or s.setPC(e)),  # 3E - "Shift value pointed by HL right"
...
CB_prefix_4x = [(1, (8,), ext_bit(0, B)),        # 40 - "Test bit 0 of B"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(0, s.ram[s.getHL()]) or s.setPC(e)),  # 46 - "Test bit 0 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(1, s.ram[s.getHL()]) or s.setPC(e)),  # 4E - "Test bit 1 of value pointed by HL"
...
CB_prefix_5x = [(1, (8,), ext_bit(2, B)),        # 50 - "Test bit 2 of B"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(2, s.ram[s.getHL()]) or s.setPC(e)),  # 56 - "Test bit 2 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(3, s.ram[s.getHL()]) or s.setPC(e)),  # 5E - "Test bit 3 of value pointed by HL"
...
CB_prefix_6x = [(1, (8,), ext_bit(4, B)),        # 60 - "Test bit 4 of B"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(4, s.ram[s.getHL()]) or s.setPC(e)),  # 66 - "Test bit 4 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(5, s.ram[s.getHL()]) or s.setPC(e)),  # 6E - "Test bit 5 of value pointed by HL"
...
CB_prefix_7x = [(1, (8,), ext_bit(6, B)),        # 70 - "Test bit 6 of B"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(6, s.ram[s.getHL()]) or s.setPC(e)),  # 76 - "Test bit 6 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.CPU_EXT_BIT(7, s.ram[s.getHL()]) or s.setPC(e)),  # 7E - "Test bit 7 of value pointed by HL"
...
CB_prefix_8x = [(1, (8,), ext_clr(0, B)),        # 80 - "Clear (reset) bit 0 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(0, s.ram[s.getHL()])) or s.setPC(e)),  # 86 - "Clear (reset) bit 0 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(1, s.ram[s.getHL()])) or s.setPC(e)),  # 8E - "Clear (reset) bit 1 of value pointed by HL"
...
CB_prefix_9x = [(1, (8,), ext_clr(2, B)),        # 90 - "Clear (reset) bit 2 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(2, s.ram[s.getHL()])) or s.setPC(e)),  # 96 - "Clear (reset) bit 2 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(3, s.ram[s.getHL()])) or s.setPC(e)),  # 9E - "Clear (reset) bit 3 of value pointed by HL"
...
CB_prefix_Ax = [(1, (8,), ext_clr(4, B)),        # A0 - "Clear (reset) bit 4 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(4, s.ram[s.getHL()])) or s.setPC(e)),  # A6 - "Clear (reset) bit 4 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(5, s.ram[s.getHL()])) or s.setPC(e)),  # AE - "Clear (reset) bit 5 of value pointed by HL"
...
CB_prefix_Bx = [(1, (8,), ext_clr(6, B)),        # B0 - "Clear (reset) bit 6 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(6, s.ram[s.getHL()])) or s.setPC(e)),  # B6 - "Clear (reset) bit 6 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_RES(7, s.ram[s.getHL()])) or s.setPC(e)),  # BE - "Clear (reset) bit 7 of value pointed by HL"
...
CB_prefix_Cx = [(1, (8,), ext_set(0, B)),        # C0 - "Set bit 0 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(0, s.ram[s.getHL()])) or s.setPC(e)),  # C6 - "Set bit 0 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(1, s.ram[s.getHL()])) or s.setPC(e)),  # CE - "Set bit 1 of value pointed by HL"
...
CB_prefix_Dx = [(1, (8,), ext_set(2, B)),        # D0 - "Set bit 2 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(2, s.ram[s.getHL()])) or s.setPC(e)),  # D6 - "Set bit 2 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(3, s.ram[s.getHL()])) or s.setPC(e)),  # DE - "Set bit 3 of value pointed by HL"
...
CB_prefix_Ex = [(1, (8,), ext_set(4, B)),        # E0 - "Set bit 4 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(4, s.ram[s.getHL()])) or s.setPC(e)),  # E6 - "Set bit 4 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(5, s.ram[s.getHL()])) or s.setPC(e)),  # EE - "Set bit 5 of value pointed by HL"
...
CB_prefix_Fx = [(1, (8,), ext_set(6, B)),        # F0 - "Set bit 6 of B"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(6, s.ram[s.getHL()])) or s.setPC(e)),  # F6 - "Set bit 6 of value pointed by HL"
                ...
                (1, (16,), lambda s, e: s.ram.__setitem__(s.getHL(), s.CPU_EXT_SET(7, s.ram[s.getHL()])) or s.setPC(e)),  # FE - "Set bit 7 of value pointed by HL"
...
if __name__ == "__main__":
    ...
    print "opcode_0x", len(opcode_0x)
    print "opcode_1x", len(opcode_1x)
    print "opcode_2x", len(opcode_2x)
    print "opcode_3x", len(opcode_3x)
    print "opcode_4x", len(opcode_4x)
    print "opcode_5x", len(opcode_5x)
    print "opcode_6x", len(opcode_6x)
    print "opcode_7x", len(opcode_7x)
    print "opcode_8x", len(opcode_8x)
    print "opcode_9x", len(opcode_9x)
    print "opcode_Ax", len(opcode_Ax)
    print "opcode_Bx", len(opcode_Bx)
    print "opcode_Cx", len(opcode_Cx)
    print "opcode_Dx", len(opcode_Dx)
    print "opcode_Ex", len(opcode_Ex)
    print "opcode_Fx", len(opcode_Fx)
    print "CB_prefix_0x", len(CB_prefix_0x)
    print "CB_prefix_1x", len(CB_prefix_1x)
    print "CB_prefix_2x", len(CB_prefix_2x)
    print "CB_prefix_3x", len(CB_prefix_3x)
    print "CB_prefix_4x", len(CB_prefix_4x)
    print "CB_prefix_5x", len(CB_prefix_5x)
    print "CB_prefix_6x", len(CB_prefix_6x)
    print "CB_prefix_7x", len(CB_prefix_7x)
    print "CB_prefix_8x", len(CB_prefix_8x)
    print "CB_prefix_9x", len(CB_prefix_9x)
    print "CB_prefix_Ax", len(CB_prefix_Ax)
    print "CB_prefix_Bx", len(CB_prefix_Bx)
    print "CB_prefix_Cx", len(CB_prefix_Cx)
    print "CB_prefix_Dx", len(CB_prefix_Dx)
    print "CB_prefix_Ex", len(CB_prefix_Ex)
    print "CB_prefix_Fx", len(CB_prefix_Fx)
    opcodes[0x7c+0xFF][2](1, 1)
