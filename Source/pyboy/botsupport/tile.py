...
class Tile:
    def __init__(self, mb, identifier=None, index=None):
        """
        The Game Boy uses tiles as the building block for all graphics on the screen. This base-class is used for `pyboy.pyboy.PyBoy.get_tile`, `pyboy.botsupport.sprite.Sprite` and `pyboy.botsupport.tilemap.TileMap`, when refering to graphics.
        ...
        The data of this class is static, apart from the image data, which is loaded from the Game Boy's memory when needed. Be aware, that the graphics for the tile can change between each call to `pyboy.pyboy.PyBoy.tick`.
        """
    ...
    def get_identifier(self):
        """
        The Game Boy has a slightly complicated indexing system for tiles. Use this attribute instead to get a unified and easy way to compare and identify tiles. If you want to modify the state of the tilemap or tile data directly in the emulator, you'll need to look at `pyboy.botsupport.tile.Tile.get_index`.
    ...
    def get_index(self):
        """
        If you need to compare two tiles, consider using `pyboy.botsupport.tile.Tile.identifier` instead, or simply the equal operator: `tile1 == tile2`. The index returned here is mostly for use when interfacing directly with the Game Boy.
        ...
        The signed indexes also uses another starting address than the unsigned indexes. Use the `pyboy.botsupport.tile.Tile.data_address` attribute if you want the address of the tile data.
