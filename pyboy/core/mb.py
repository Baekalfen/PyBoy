...
class Motherboard:
    ...
    def breakpoint_find(self, bank, addr):
        if (bank, addr) in self.breakpoints:
            return [bank, addr, self.breakpoints[(bank, addr)]]
        return []

    def breakpoint_remove(self, bank, addr):
        logger.debug(f"Breakpoint remove: ({bank}, {addr})")
        if (bank, addr) in self.breakpoints:
            opcode = self.breakpoints[(bank, addr)]
            del self.breakpoints[(bank, addr)]
    ...
    def breakpoint_reached(self):
        ...
        if pc < 0x4000 and not self.bootrom_enabled:
            ...
            bank = -1
        if (bank, pc) in self.breakpoints:
            opcode = self.breakpoints[(bank, pc)]
            # Breakpoint hit
            addr = pc
