...
\begin{description}
    ...
    \item [Look at the current implementation]
        Have a look at the `api` module and the "game wrappers" in PyBoy and see if it has the needed functionality. If it doesn't, it is easy to extend it. There is also a crude Tetris and Mario example in the repo to get inspiration from.
...
\begin{description}
    ...
    \item [Look at the current implementation]
        Have a look at the `api` module in PyBoy and see if it has the needed functionality. If it doesn't, it is easy to extend it. There is also a crude Tetris bot to get inspiration from.

    \item [Figure out a method]
        Figure out how to best represent the data on the screen for the AI/bot developer. Find the best performing way to extract the necessary information from PyBoy -- use the api module as much as possible. If PyBoy lacks the necessary features, figure out how to build an extension.
